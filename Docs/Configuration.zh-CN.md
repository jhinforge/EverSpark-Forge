...
DeepSeek 复用 OpenAI 格式的 Chat Completions 适配器。点击 **Fill DeepSeek settings** 可填入目前官方的 `https://api.deepseek.com/v1` 地址和 `deepseek-flash` 模型；自行填写 DeepSeek 官方 API Key，测试、保存后在 Forge 选择该服务。响应模式由系统自动处理；以后配置时请核对 DeepSeek 官方的最新模型 ID。
